...
## Instructions
- Build the library: `make` (defaults to `BUFFER_SIZE=42`, override with `make BUFFER_SIZE=1024`).
...
## Algorithm
The function keeps a static stash of unread data, appending new `read` chunks until a newline or EOF is encountered. It slices and returns the next line, then trims the stash to retain only remaining data. This approach minimizes reads, avoids global variables, and frees intermediate buffers to prevent leaks.
